...
class Minifier:
    ...
    def emit_tokens_to_rewritten_glsl(self, out, *, preserve_exported_switches, inside_directive=False):
        ...
        for tok in self.tokens:
            ...
            is_top_level_decl_after_block = (
                not args.human_readable
            ...
            needs_separator_after_macro_call = (
                lasttoken.type == "OP"
                and lasttoken.value == ")"
                and tok.type in ["FLOAT", "INT", "HEX", "ID", "DEFINED_ID"]
            )

            if (is_directive or is_top_level_decl_after_block) and not is_newline:
                out.write('\n')
            elif needs_whitespace and (
                lasttoken_needs_whitespace or needs_separator_after_macro_call
            ):
                # Keep a token boundary after function-like macro invocations such as
                # OUT(float2) value, which some GLSL preprocessors misparse when minified to
                # OUT(float2)value.
                out.write(' ')
