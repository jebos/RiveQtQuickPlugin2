...
#if defined(GL_ES)
#define RIVE_GLSL_HAS_EXPLICIT_BINDINGS (@GLSL_VERSION >= 310)
#else
#define RIVE_GLSL_HAS_EXPLICIT_BINDINGS (@GLSL_VERSION >= 420)
#endif
...
#if RIVE_GLSL_HAS_EXPLICIT_BINDINGS
...
#ifdef @VERTEX
#if defined(@TARGET_VULKAN) || (defined(GL_ES) && @GLSL_VERSION >= 310)
...
#elif RIVE_GLSL_HAS_EXPLICIT_BINDINGS
#define TEXTURE_RGBA32UI(SET, IDX, NAME)                                       \
...
#if !defined(GL_ES) && @GLSL_VERSION < 420
#ifdef GL_ARB_shading_language_packing
#extension GL_ARB_shading_language_packing : require
#else
INLINE float rive_half_to_float(uint halfBits)
{
    uint sign = (halfBits & 0x8000u) << 16;
    ...
    if (exponent == 0u)
    {
        ...
        return uintBitsToFloat(sign | (exponent << 23) | (mantissa << 13));
    }
...
INLINE uint rive_float_to_half(float value)
{
    ...
    if (exponent <= 0)
    {
        ...
        mantissa = (mantissa | 0x00800000u) >> uint(1 - exponent);
    ...
    if (exponent >= 31)
    {
        ...
        return sign | 0x7c00u | (mantissa >> 13) | 1u;
    }
...
INLINE float2 unpackHalf2x16(uint u)
{
    return float2(rive_half_to_float(u & 0xffffu),
                  rive_half_to_float(u >> 16));
}

INLINE uint packHalf2x16(float2 v)
{
    return rive_float_to_half(v.x) | (rive_float_to_half(v.y) << 16);
}
#endif
#endif
